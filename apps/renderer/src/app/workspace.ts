...
function profileInstructions(employee: Employee, extra = '') {
  ...
  return `You are EasyAI's digital employee "${role}" (${employee.id}).${roleBrief} ${focus} Reply in the user's language. ${extra}`.trim();
}
...
export function useWorkspace() {
   ...
   * inside Electron (window.easyaiDesktop present) and the caller does not
   * request legacy collaborator briefs.
   * ------------------------------------------------------------------ */

  const serverChatActive = () => Boolean(window.easyaiDesktop);
  ...
  async function serverChatTurn(
    ...
    try {
      ...
      if (!context.mcpConnections.length) {
        console.warn('[easyai] chat context has 0 MCP connectors; server may backfill from KV');
      } else {
        console.info(`[easyai] chat context MCP connectors: ${context.mcpConnections.map((item) => item.name).join(', ')}`);
      }
