...
import type { KnowledgeBaseRuntime, ModelConfig } from '@easyai/contracts';
...
function knowledgeRoot() {
  return process.env.EASYAI_KNOWLEDGE_DIR?.trim() || path.resolve(process.cwd(), '.easyai-knowledge');
}
...
export async function ingestKnowledge(input: {
  ...
  if (input.kb.provider === 'bailian') {
    ...
    if (!categoryId) {
      throw new Error('Bailian upload requires categoryId. Recreate the knowledge base from EasyAI or set categoryId on the base.');
    }
    let bytes: Buffer;
