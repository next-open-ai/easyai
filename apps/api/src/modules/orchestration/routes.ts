...
import { JsonFileStore, Orchestrator, createScriptedRunner, type AgentRunner, type OrcEvent } from '@easyai/orchestrator';
import type { ChatRunContext, ConfirmProjectInput, CreateProjectDraftInput, ProjectTask, ResolveProjectApprovalInput } from '@easyai/orchestrator';
 ...
 * (`EASYAI_DATA_DIR`, defaults to `~/.easyai`). Secrets never reach this store.
 */
...
/**
 * Headless smoke mode: `EASYAI_ORCH_RUNNER=memory-echo|memory-approval` swaps
...
function scriptedRunner(): AgentRunner | null {
  const mode = process.env.EASYAI_ORCH_RUNNER;
