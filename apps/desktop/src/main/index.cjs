...
const apiPort = Number(process.env.EASYAI_API_PORT || 4318);
let mainWindow;
let apiProcess;
let gatewayProcess = null;
let database;
const storageRoot = () => path.join(app.getPath('home'), '.easyai');
const databaseFile = () => path.join(storageRoot(), 'easyai.sqlite');
...
function registerPreviewRoot(root) {
  ...
  for (const [token, existing] of previewRoots.entries()) {
    if (existing === resolved) return { token, origin: `easyai-preview://${token}` };
  }
  const token = randomUUID().replace(/-/g, '').slice(0, 16);
  previewRoots.set(token, resolved);
  return { token, origin: `easyai-preview://${token}` };
}
...
async function importGitSkillRepository(value) {
  ...
  const temporaryRoot = mkdtempSync(path.join(tmpdir(), 'easyai-skill-'));
...
function startApi() {
  apiProcess = fork(apiEntry(), [], {
    env: {
      ...
      EASYAI_API_PORT: String(apiPort),
      EASYAI_DATA_DIR: storageRoot(),
      EASYAI_SKILLS_DIR: path.join(storageRoot(), 'skills'),
      EASYAI_WORKSPACES_DIR: path.join(storageRoot(), 'workspaces'),
      EASYAI_KNOWLEDGE_DIR: path.join(storageRoot(), 'knowledge'),
      EASYAI_EXPERIENCE_DIR: path.join(storageRoot(), 'experience'),
  ...
  apiProcess.on('message', (message) => {
    ...
    if (payload.type !== 'easyai:secrets:request') return;
    pushSecretsToApi();
  });
...
async function startGatewayIfEnabled() {
  ...
  gatewayProcess = fork(gatewayEntry(), [], {
    env: {
      ...process.env,
      EASYAI_API_URL: `http://127.0.0.1:${apiPort}/api/orch`,
    },
    stdio: 'inherit',
  });
  ...
  gatewayProcess.on('message', (message) => {
    if (!message || typeof message !== 'object') return;
    const payload = /** @type {{type?: string}} */ (message);
    if (payload.type !== 'easyai:channels:secrets:request') return;
    gatewayProcess.send?.({
      type: 'easyai:channels:secrets',
...
async function waitForApi() {
  ...
  throw new Error('EasyAI local API did not become ready.');
}
...
async function createWindow() {
  ...
  const rendererUrl = process.env.EASYAI_RENDERER_URL;
...
async function runEnvironmentChecks(onProgress) {
  ...
  // 数据目录可写（~/.easyai）
  await begin('storage', '本地数据目录（~/.easyai）', '可读写');
  let storageOk = true;
  let storageError = '';
  try {
  ...
  await finish({
    id: 'storage', name: '本地数据目录（~/.easyai）', status: storageOk ? 'ok' : 'error', required: '可读写',
...
app.whenReady().then(async () => {
  protocol.handle('easyai-preview', async (request) => {
    try {
  ...
  startApi();
  ipcMain.handle('easyai:pick-file', async () => {
    const result = await dialog.showOpenDialog(mainWindow, { properties: ['openFile'] });
    return result.canceled ? null : result.filePaths[0];
  });
  ipcMain.handle('easyai:pick-project-directory', async () => {
    const result = await dialog.showOpenDialog(mainWindow, { title: '选择项目空间目录', properties: ['openDirectory', 'createDirectory'] });
    return result.canceled ? null : result.filePaths[0];
  });
  ipcMain.handle('easyai:create-project-workspace', (_, value) => createProjectWorkspace(value));
  ipcMain.handle('easyai:list-project-files', (_, root) => listProjectFiles(root));
  ipcMain.handle('easyai:read-project-file', (_, root, relative) => readProjectFile(root, relative));
  ipcMain.handle('easyai:write-project-file', (_, root, relative, content) => writeProjectFile(root, relative, content));
  ipcMain.handle('easyai:sync-project-workspace', (_, root, runId) => syncRunWorkspaceToProject(root, runId));
  ipcMain.handle('easyai:materialize-project-assets', (_, root, assetIds) => materializeProjectAssets(root, assetIds));
  ipcMain.handle('easyai:register-preview-root', (_, root) => registerPreviewRoot(root));
  ipcMain.handle('easyai:register-asset-preview-root', (_, assetId) => registerAssetPreviewRoot(assetId));
  ipcMain.handle('easyai:read-project-preview', (_, root, relative) => readProjectPreview(root, relative));
  ipcMain.handle('easyai:read-asset-preview', (_, assetId) => readAssetPreview(assetId));
  ipcMain.handle('easyai:reveal-project-file', (_, root, relative) => revealProjectFile(root, relative));
  ipcMain.handle('easyai:open-asset-in-browser', (_, assetId) => openAssetInBrowser(assetId));
  ipcMain.handle('easyai:open-project-file-in-browser', (_, root, relative) => openProjectFileInBrowser(root, relative));
  ipcMain.handle('easyai:pick-skill', async () => {
  ...
  ipcMain.handle('easyai:create-skill', (_, value) => {
    const name = safeSkillName(value?.name);
  ...
  ipcMain.handle('easyai:write-skill-draft', (_, value) => writeSkillDraft(value));
  ipcMain.handle('easyai:read-skill-draft', (_, file) => readSkillDraft(file));
  ipcMain.handle('easyai:list-skill-files', (_, file) => listSkillFiles(file));
  ipcMain.handle('easyai:read-skill-file', (_, file) => readManagedSkillFile(file));
  ipcMain.handle('easyai:write-skill-file', (_, file, content) => writeManagedSkillFile(file, content));
  ipcMain.handle('easyai:delete-managed-skill', (_, file) => deleteManagedSkill(file));
  ipcMain.handle('easyai:install-skill', async (_, reference) => runSkillsCli(reference));
  ipcMain.handle('easyai:import-git-skill', async (_, url) => importGitSkillRepository(url));
  ipcMain.handle('easyai:find-skills', async (_, query, batchCount) => findSkills(query, batchCount));
  ipcMain.handle('easyai:open-external', (_, value) => shell.openExternal(String(value)));
  ipcMain.handle('easyai:get-model-config', () => readModelConfig());
  ipcMain.handle('easyai:save-model-config', (_, value) => writeModelConfig(value));
  ipcMain.handle('easyai:get-search-config', () => readSearchConfig());
  ipcMain.handle('easyai:save-search-config', (_, value) => writeSearchConfig(value));
  ipcMain.handle('easyai:test-provider', (_, value) => testProviderConnection(value));
  ipcMain.handle('easyai:list-provider-models', (_, value) => listProviderModels(value));
  ipcMain.handle('easyai:list-ollama-models', async (_, baseUrl) => {
    return listProviderModels({ type: 'ollama', baseUrl });
  });
  ipcMain.handle('easyai:pull-ollama-model', async (_, baseUrl, modelName) => {
  ...
  ipcMain.handle('easyai:storage-get', async (_, key) => apiKvGet(key));
  ipcMain.handle('easyai:storage-set', async (_, key, value) => { await apiKvSet(key, value); });
  ipcMain.handle('easyai:get-channel-settings', () => getChannelSettings());
  ipcMain.handle('easyai:save-channel-settings', async (_, payload) => saveChannelSettings(payload));
  ipcMain.handle('easyai:gateway-status', () => gatewayStatus());
  ipcMain.handle('easyai:env-check', async (event) => runEnvironmentChecks((payload) => event.sender.send('easyai:env-check-progress', payload)));
  ipcMain.handle('easyai:gateway-restart', () => gatewayRestart());
  ipcMain.handle('easyai:list-assets', () => listAssets());
  ipcMain.handle('easyai:archive-artifact', (_, value) => archiveArtifact(value));
  ipcMain.handle('easyai:link-assets-to-project', (_, value) => linkAssetsToProject(value));
  ipcMain.handle('easyai:unlink-assets-from-project', (_, assetIds) => unlinkAssetsFromProject(assetIds));
  ipcMain.handle('easyai:save-asset', async (_, assetId) => {
    const { row, target } = assetFile(assetId);
  ...
  ipcMain.handle('easyai:reveal-asset', (_, assetId) => { const { target } = assetFile(assetId); shell.showItemInFolder(target); });
