...
把外部 IM（一期：Telegram）经一个独立**网关子进程**接入 EasyAI 本地服务：消息走通道协议 →
...
- 配置：`EASYAI_GATEWAY_CONFIG` 文件 或 orchestrator KV `channels.v1`；无启用通道即退出；
...
- 主进程启动（api 就绪、域迁移后）读 `channels.v1`：有任一 enabled 通道 → fork 网关子进程
  （env `EASYAI_API_URL`），退出时 `SIGTERM`；
...
| 5 | typecheck/build：channel、gateway、api、desktop | `corepack pnpm --filter @easyai/<p> …` | ✅ |

## 5. 真机 Telegram 联调步骤（需真实 bot token）

1. `pnpm build`（含 channel/gateway）；
2. 起桌面：`pnpm dev`（api 4318 + 域 KV 就绪）；或在 headless 下另起 api：
   `EASYAI_ORCH_RUNNER=…` 不建议，生产应起真实模型（桌面 keyring 提供）；
3. 写入配置：
   `EASYAI_TG_BOT_TOKEN_FILE=/path/token EASYAI_GATEWAY_ALLOW="telegram:user:<你的tg用户id>" node scripts/gateway-config.mjs`
