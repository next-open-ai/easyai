...
const mode = ref<LibraryMode>((localStorage.getItem('easyai.assets.mode') as LibraryMode) || 'projects');
...
watch(mode, (value) => localStorage.setItem('easyai.assets.mode', value));
watch(
  filteredProjects,
  (list) => {
    if (!list.length) {
  ...
  <section class="flex h-full min-h-0 flex-col overflow-hidden">
    <div class="mx-auto flex h-full min-h-0 w-full max-w-[1400px] flex-col px-6 py-8 sm:px-10">
      <header class="shrink-0">
        <p class="text-[11px] font-extrabold tracking-[.13em] text-[var(--accent)]">EasyAI / ASSETS</p>
