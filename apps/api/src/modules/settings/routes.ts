...
function dataDir(): string {
  return process.env.EASYAI_DATA_DIR || path.join(os.homedir(), '.easyai');
}
...
function splitSearchSettings(value: unknown): SettingsEnvelope {
  ...
  return {
    ...
    secrets: { providers: Object.fromEntries(providers.map((provider) => [String(provider.id || ''), { apiKey: String(provider.apiKey || '') }])) },
  };
}
...
function normalizeCapabilitySkills(value: unknown) {
  return arrayOfRecords(value).map((item) => ({
    ...
    systemOnly: Boolean(item.systemOnly),
    execution: {
      allowWorkspaceWrite: Boolean(asRecord(item.execution).allowWorkspaceWrite),
      allowScriptExecution: Boolean(asRecord(item.execution).allowScriptExecution),
      allowedNetworkHosts: Array.isArray(asRecord(item.execution).allowedNetworkHosts)
        ? (asRecord(item.execution).allowedNetworkHosts as unknown[]).map((host) => String(host))
        : [],
      allowAllNonDestructive: Boolean(asRecord(item.execution).allowAllNonDestructive),
    },
  }));
}

function normalizeCapabilityPolicies(value: unknown) {
  return arrayOfRecords(value).map((item) => ({
...
export const settingsRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/settings/capabilities/skills', async () => normalizeCapabilitySkills(readJson('capability-skills')));
  app.put('/settings/capabilities/skills', async (request) => writeJson('capability-skills', normalizeCapabilitySkills(request.body ?? [])));

  app.get('/settings/capabilities/policies', async () => normalizeCapabilityPolicies(readJson('capability-policies')));
  app.put('/settings/capabilities/policies', async (request) => writeJson('capability-policies', normalizeCapabilityPolicies(request.body ?? [])));
};
