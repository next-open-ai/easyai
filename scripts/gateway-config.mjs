 ...
 *   EASYAI_TG_BOT_TOKEN="123:ABC" \
 *   EASYAI_GATEWAY_ALLOW="telegram:user:111111,telegram:chat:-100222222" \
 *   EASYAI_DEFAULT_EMPLOYEE=general \
 *   node scripts/gateway-config.mjs
 *
 * Reads token from EASYAI_TG_BOT_TOKEN_FILE when the token itself is set there
 * instead (avoids pasting secrets into shell history).
 */
const port = process.env.EASYAI_API_PORT || '4318';
const base = `http://127.0.0.1:${port}/api/orch`;

import { readFileSync } from 'node:fs';

function token() {
  if (process.env.EASYAI_TG_BOT_TOKEN) return process.env.EASYAI_TG_BOT_TOKEN;
  const file = process.env.EASYAI_TG_BOT_TOKEN_FILE;
...
if (!botToken) {
  console.error('缺少 Telegram bot token：设置 EASYAI_TG_BOT_TOKEN 或 EASYAI_TG_BOT_TOKEN_FILE。');
  process.exit(2);
}
const allowlist = (process.env.EASYAI_GATEWAY_ALLOW ?? '')
  .split(',')
...
const config = {
  version: 1,
  apiBaseUrl: base,
  defaultEmployeeId: process.env.EASYAI_DEFAULT_EMPLOYEE || 'general',
  channels: {
