...
async function persist(value = current.value) {
  ...
  try {
    if (isDraft.value || selectedFile.value === 'SKILL.md') {
      const name = skillName(value); const manifest = await writeSkillDraft({ name, content: value }); if (!manifest) throw new Error('Unable to save SKILL.md.');
...
async function acceptDiff() { await persist(proposed.value); current.value = proposed.value; const root = selectedPath.value.replace(/[\\/][^\\/]+$/, ''); for (const file of pendingFiles.value) await writeSkillFile({ path: `${root}/${file.relative}`, content: file.content }); await loadFiles(); proposed.value = ''; pendingFiles.value = []; diff?.dispose(); diff = undefined; await restoreEditor(); }
