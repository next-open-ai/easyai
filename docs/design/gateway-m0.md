# EasyAI 通道网关 · M0 设计与实现记录

> 状态：M0 核心已实现（`@easyai/orchestrator` + api orchestration 模块 + desktop 域存储转发）。
...
- 桌面主进程 `easyai:storage-get/set` 转发到 `/api/orch/kv`；启动时一次性
...
- `apps/api/src/modules/orchestration/context-assembler.ts`：`resolveTaskContext(store, task)`
  ...
  [含 `easyai-workspace` harness]/search/mcp/kb）。缺失项优雅降级；无模型则任务失败并
...
- 密钥通道：`secrets.ts` → api 子进程启动后经 fork IPC 向 Electron 主进程请求一次性
  ...
  本地验收可用 `EASYAI_SECRETS_FILE` 预置同构 JSON（桌面运行不经过此路径）。
...
`EASYAI_SECRETS_FILE` 载入 keyring 后，远程终端仅 `POST /projects` + `confirm {}`，
服务端组装器自动解析模型（ollama/chat-x）、注入 `easyai-workspace` harness 并完成
...
api 提供 `EASYAI_ORCH_RUNNER=memory-echo|memory-approval` 确定性 runner
