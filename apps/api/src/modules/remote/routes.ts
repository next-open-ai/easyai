...
function gatewayApiUrl(): string {
  const port = Number(process.env.EASYAI_API_PORT ?? 4318);
...
async function startGatewayIfEnabled(): Promise<{ running: boolean; pid: number | null }> {
  ...
  gatewayProcess = fork(entry, [], {
    ...
    env: {
      ...process.env,
      EASYAI_API_URL: gatewayApiUrl(),
      EASYAI_GATEWAY_CONFIG: runtimeGatewayConfigFile(),
    },
    stdio: 'inherit',
  });
...
export const remoteRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/remote/settings', async () => ({ meta: await readChannelMeta(), secrets: readChannelSecrets() }));

  app.put('/remote/settings', async (request) => {
