 ...
 *   EASYAI_API_URL=<orchestrator base>   (defaults to the local api)
 *   EASYAI_GATEWAY_CONFIG=<json file>    (optional override)
 *   EASYAI_GATEWAY_STUB=1                (offline stub channel; acceptance only)
...
async function main(): Promise<void> {
  const apiBaseUrl = process.env.EASYAI_API_URL ?? 'http://127.0.0.1:4318/api/orch';
  const fileConfig = loadConfigFile(process.env.EASYAI_GATEWAY_CONFIG);
  ...
  const config: GatewayConfig = {
    ...
    ...kvConfig,
    ...fileConfig,
    ...(process.env.EASYAI_GATEWAY_STUB === '1' ? { allowAll: true } : {}),
    channels: {
      telegram: {
  ...
  const gateway = createGateway(config, {
    stub: process.env.EASYAI_GATEWAY_STUB === '1',
    fakeFeishu: process.env.EASYAI_FEISHU_FAKE === '1',
  });
