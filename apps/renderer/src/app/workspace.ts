...
function profileInstructions(employee: Employee, extra = '') {
  ...
  return `You are EasyAI's digital employee "${role}" (${employee.id}). ${focus} Reply in the user's language. ${extra}`.trim();
}
...
export function useWorkspace() {
  ...
  const generateProjectDraft = async (goal: string, model: ProviderConfig): Promise<ProjectTaskDraft[]> => {
    ...
    const instruction = `You are EasyAI's project coordinator. Decompose the goal below into 2-5 independent, parallelizable tasks. Return ONLY a JSON array. Each item must be {"title": string, "objective": string, "employeeId": one of [${roster}], "skillIds": string[]}. Do not create dependent tasks. Goal: ${goal}`;
