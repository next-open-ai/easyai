...
## A. 服务端可复用编排层（`@easyai/orchestrator`，由 apps/api 托管）
...
| B3 | `EASYAI_DATA_DIR` 注入 + `domain.json` 持久化 | 起服后检查 `~/.easyai/domain.json` | ✅ |
| B4 | 密钥不落盘：fork IPC keyring（`easyai:secrets` 一次性解密快照） | `secrets.ts` + 主进程 `message` 应答 | ✅ |
...
| C3 | 无头网关冒烟（echo/approval 双 runner） | `EASYAI_ORCH_RUNNER=… node dist/main.cjs` + `scripts/headless-gateway-smoke.mjs` | ✅ ALL PASS |
| C4 | 远程 confirm 无 context（服务端组装：KV + keyring） | `EASYAI_SECRETS_FILE=…` + `scripts/remote-project-confirm.mjs` | ✅ ALL PASS |

## D. 构建与类型

| # | 验收点 | 验证入口 | 状态 |
|---|---|---|---|
| D1 | orchestrator / api / desktop typecheck | `corepack pnpm --filter @easyai/<p> typecheck` | ✅ |
| D2 | renderer `vue-tsc -b`（历史错误已修复：bundler 解析 + kb/import 修复） | `cd apps/renderer && corepack pnpm exec vue-tsc -b` | ✅ |
| D3 | 逐包 build（contracts/tools/storage/agent-core/orchestrator/api/renderer） | `corepack pnpm --filter @easyai/<p> build` | ✅ |
...
| E3 | HTTP 验收：chat 无 context + 审批 allow（无 resumeContext）自动续跑 | `scripts/remote-chat.mjs`（memory-approval + `EASYAI_SECRETS_FILE`） | ✅ ALL PASS |
