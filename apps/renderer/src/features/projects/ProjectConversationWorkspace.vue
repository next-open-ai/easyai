...
const TREE_WIDTH_KEY = "easyai.project.tree-width";
const TREE_COLLAPSED_KEY = "easyai.project.tree-collapsed";
...
/** Short parent hint, e.g. ~/.easyai/projects */
const workspaceLocationHint = computed(() => {
  const raw = props.project.workspacePath?.trim() || '';
  if (!raw) return '';
  const normalized = raw.replace(/\\/g, '/');
  const projectsMarker = '/.easyai/projects/';
  if (normalized.includes(projectsMarker)) return '~/.easyai/projects';
...
async function openWorkspaceFolder() {
  if (!props.project.workspacePath || !window.easyaiDesktop) return;
  try {
    await window.easyaiDesktop.revealProjectFile(props.project.workspacePath, '');
  } catch {
...
async function recoverProjectFiles() {
  ...
  try {
    const catalog = await window.easyaiDesktop.listAssets();
  ...
  for (const runId of runIds) {
    try { await window.easyaiDesktop.syncProjectWorkspace(props.project.workspacePath, runId); } catch { /* Continue other run workspaces. */ }
  }
  if (assetIds.length) {
    try { await window.easyaiDesktop.materializeProjectAssets(props.project.workspacePath, assetIds); } catch { /* Keep listing whatever already synced. */ }
  }
}
...
async function loadFiles(options: { recover?: boolean } = {}) {
  ...
  try {
    ...
    files.value = await window.easyaiDesktop.listProjectFiles(props.project.workspacePath);
...
async function syncRunWorkspacesIntoProject() {
  ...
  for (const runId of runIds) {
    try { await window.easyaiDesktop.syncProjectWorkspace(props.project.workspacePath, runId); } catch { /* 单个运行失败不影响其它 */ }
  }
}
...
async function selectFile(entry: FileEntry) {
  ...
  const file = await window.easyaiDesktop?.readProjectFile(props.project.workspacePath, entry.relative);
  if (!file) return;
...
async function saveFile() { if (!props.project.workspacePath || !selectedFile.value || !editor) return; const result = await window.easyaiDesktop?.writeProjectFile(props.project.workspacePath, selectedFile.value, editor.getValue()); if (result) fileContent.value = result.content; }
        ...
        <div class="min-h-0 overflow-y-auto p-3">
          ...
          >
            <span class="relative grid h-10 w-10 place-items-center rounded-xl text-[10px] font-bold text-white" :style="{ background: employee.color }">
              <i v-if="statusFor(employee.id) === 'running'" class="easyai-run-spin" />
              <i v-if="statusFor(employee.id) === 'running'" class="easyai-run-pulse" />
              {{ employee.initials }}
...
.easyai-running-pill {
  ...
  color: #fff;
  animation: easyai-shimmer 1.7s linear infinite;
}
.easyai-run-spin,
.easyai-run-pulse {
  position: absolute;
...
.easyai-run-spin {
  ...
  animation: easyai-spin 1.05s linear infinite;
}
.easyai-run-pulse {
  box-shadow: 0 0 0 0 rgba(99, 102, 241, 0.7);
  animation: easyai-pulse 1.7s ease-out infinite;
}
@keyframes easyai-shimmer {
  to {
