import type { ChatRunContext, KeyValueStore, ProjectTask } from '@easyai/orchestrator';
...
const BASELINE_WORKSPACE_SKILL_ID = 'easyai-workspace';
const BASELINE_WORKSPACE_INSTRUCTIONS = `---
name: easyai-workspace
description: EasyAI platform workspace harness for isolated read/write and script execution in the current run directory.
---

You are using the **EasyAI workspace harness** (easyai-workspace). It is always authorized for this run. Use it for artifacts that belong in the run workspace, not on arbitrary host paths.
...
function profileFor(
  ...
  const instructions = `${name}（EasyAI 数字员工）。${roleBrief} ${focus}`.trim();
