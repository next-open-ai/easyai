# EasyAI M2 「远程办公」门户 / 飞书 / 远程中继 · 设计与实现记录
...
- 主进程 `apps/desktop/src/main/index.cjs`：
  ...
  - IPC：`easyai:get-channel-settings`、`save-channel-settings`、`gateway-status`、`gateway-restart`；
  - 网关子进程 fork 后应答 `easyai:channels:secrets:request`（一次性下发解密凭证）。
