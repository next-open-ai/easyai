...
export async function startWebLauncher(projectRoot, options = {}) {
  ...
  const requestedPort = Number(options.port || process.env.EASYAI_API_PORT || '4318');
  const host = options.host || process.env.EASYAI_API_HOST || '127.0.0.1';
  const dataDir = options.dataDir || defaultDataDir();
  if (!existsSync(entry)) throw new Error(`Missing API build: ${entry}. Run \`pnpm build\` first.`);
  if (!existsSync(path.join(staticRoot, 'index.html'))) throw new Error(`Missing renderer build: ${staticRoot}/index.html. Run \`pnpm --filter @easyai/renderer build\` first.`);
  ...
  const child = spawn(process.execPath, [entry], {
    ...
    env: {
      ...process.env,
      EASYAI_API_PORT: String(port),
      EASYAI_API_HOST: host,
      EASYAI_WEB_STATIC_DIR: staticRoot,
      EASYAI_DATA_DIR: dataDir,
    },
  });
  child.once('spawn', () => {
    process.stdout.write(`EasyAI Web is starting at http://${host}:${port}\n`);
  });
