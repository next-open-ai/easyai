...
async function loadFiles() { if (!props.skill.path) return; files.value = await window.easyaiDesktop?.listSkillFiles(props.skill.path) || []; }
async function selectFile(entry: FileEntry) {
  if (entry.type === 'directory' || entry.relative === selectedFile.value || !entry.path) return;
  try { const result = await window.easyaiDesktop?.readSkillFile(entry.path); if (result) { selectedPath.value = result.path; setEditorContent(result.content, entry.relative); } } catch (cause) { error.value = cause instanceof Error ? cause.message : String(cause); }
}
...
async function persist(value = current.value) {
  ...
  try {
    if (isDraft.value || selectedFile.value === 'SKILL.md') {
      const name = skillName(value); const manifest = await window.easyaiDesktop?.writeSkillDraft({ name, content: value }); if (!manifest) throw new Error('Unable to save SKILL.md.');
      isDraft.value = false; selectedPath.value = manifest.path; files.value = await window.easyaiDesktop?.listSkillFiles(manifest.path) || [];
      emit('saved', { ...props.skill, id: `local:${name}`, name, path: manifest.path, source: 'local', description: description(value) });
    } else if (selectedPath.value) await window.easyaiDesktop?.writeSkillFile(selectedPath.value, value);
  } catch (cause) { error.value = cause instanceof Error ? cause.message : String(cause); }
  finally { saving.value = false; }
}
async function acceptDiff() { await persist(proposed.value); current.value = proposed.value; const root = selectedPath.value.replace(/[\\/][^\\/]+$/, ''); for (const file of pendingFiles.value) await window.easyaiDesktop?.writeSkillFile(`${root}/${file.relative}`, file.content); await loadFiles(); proposed.value = ''; pendingFiles.value = []; diff?.dispose(); diff = undefined; await restoreEditor(); }
...
<template>
  <section class="flex h-full min-h-0 flex-col bg-[var(--background)]"><header class="flex shrink-0 items-center justify-between border-b border-[var(--border)] px-6 py-4"><div><p class="text-[11px] font-extrabold tracking-[.12em] text-[var(--accent)]">EasyAI / SKILL WORKSPACE</p><h1 class="mt-1 text-lg font-bold">{{ skill.name }} <span class="font-normal text-[var(--muted)]">/ {{ selectedFile }}</span></h1></div><div class="flex items-center gap-3"><span v-if="isDraft" class="rounded-full bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-700">未保存草案</span><button class="rounded-lg border border-[var(--border)] px-3 py-2 text-sm font-semibold" @click="emit('close')">关闭</button><button class="rounded-lg bg-[var(--accent)] px-3 py-2 text-sm font-semibold text-white disabled:opacity-40" :disabled="saving" @click="persist()">{{ saving ? '正在保存…' : '保存 Skill' }}</button></div></header>
