...
async function refreshStatus() {
  const status = await window.easyaiDesktop?.gatewayStatus?.().catch(() => ({ running: false, pid: null }));
  gatewayRunning.value = Boolean(status?.running);
  gatewayPid.value = status?.pid ?? null;
}

async function load() {
  const settings = await window.easyaiDesktop?.getChannelSettings?.().catch(() => undefined);
...
async function save() {
  ...
  try {
    ...
    const result = await window.easyaiDesktop?.saveChannelSettings?.(payload);
...
async function restart() {
  ...
  message.value = '';
  try {
    const result = await window.easyaiDesktop?.gatewayRestart?.();
