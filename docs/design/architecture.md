...
EasyAI 是**本地优先的数字员工工作台**：不是单一聊天工具，而是把「对话 / 项目编排 / Skills / 知识库 / 自动化 / 资产」组织在**数字员工**职责之下，并通过**本地通道网关**让外部 IM（Telegram/飞书）与**远程中继终端**也能调度同一套能力。

工程上最重要的三个约定：

1. **编排在服务端，UI 只是客户端**：会话、运行、审批、项目状态机都归 `@easyai/orchestrator` 所有（由 `apps/api` 进程托管），桌面渲染层与未来终端走同一组 `/api/orch` REST/SSE；
2. **域数据单写者**：domain KV 只由 api 进程写入（`~/.easyai/domain.json`）；Electron 主进程仅保留密钥与资产元数据；
3. **分层依赖单向**：renderer 不 import Electron/Node；只有 `packages/agent-core` 能调用 Vercel AI SDK；通道协议层（`@easyai/channel`）与传输实现解耦。
...
│  │   ├─ /api/orch/**     (REST + SSE)                           │           │
│  │   └─ @easyai/orchestrator   ← 编排层（唯一运行层）              │           │
│  └───────────────────────────────▲──────────────────────────────┘           │
│                                  │ /api/orch                                │
│  apps/gateway ── fork 拉起 ──────┘    @easyai/channel 协议                   │
...
## 5. `@easyai/orchestrator` 主要逻辑（M0 核心）
...
| 层 | 路径 | 可见性 | 用途 |
| --- | --- | --- | --- |
| Run workspace | `~/.easyai/workspaces/<runId>/` | 不对用户文件树展示 | 过程脚本、`.py`/`.sh`、临时生成器 |
| Project workspace | `~/.easyai/projects/<name>-<id>/` | 左侧「项目文件」树 | 最终交付（HTML/CSS/JS/文案/图片…） |
...
- 模型/搜索/通道 token：Main sql.js + `safeStorage`；经 fork IPC（`easyai:secrets` / `easyai:channels:secrets`）一次性下发给子进程，绝不落盘 domain.json。
