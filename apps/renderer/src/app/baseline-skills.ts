...
export const BASELINE_WORKSPACE_SKILL_ID = 'easyai-workspace';

const BASELINE_INSTRUCTIONS = `---
name: easyai-workspace
description: EasyAI platform workspace harness for isolated read/write and script execution in the current run directory, plus optional publish into a shared project workspace.
---

You are using the **EasyAI workspace harness** (\`easyai-workspace\`). It is always authorized for this run.
...
## Tools (always pass skillId \`easyai-workspace\` unless noted)
